...
const SPEED_META = [
    { key: 'slow'   as const, label: 'Slow',   desc: 'Lower priority',  amount: (e: FeeEstimates) => e.low    },
    { key: 'normal' as const, label: 'Normal', desc: 'Recommended',      amount: (e: FeeEstimates) => e.medium },
    { key: 'fast'   as const, label: 'Fast',   desc: 'Higher priority',  amount: (e: FeeEstimates) => e.high   },
];
...
export function NetworkFeeSheet({
    ...
    const customNative = isEvm && evmGasOpts ? Number(customWei * evmGasOpts.gasLimit) / 1e18 : 0;
    const customUsd   = customNative && ethPriceUsd ? customNative * ethPriceUsd : 0;
        ...
        <div className="nfs-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            ...
            <div className="nfs-body">
                {SPEED_META.map(({ key, label, desc, amount }) => {
                    ...
                    const gweiStr = isEvm && evmGasOpts
                            ...
                            <div className="nfs-pricing">
                                ...
                                {(usd !== null || gweiStr) && (
                                    <span className="nfs-price-secondary">
                                        {gweiStr}{gweiStr && usd !== null ? ' · ' : ''}{usd !== null ? `≈ $${usd.toFixed(2)}` : ''}
                                    </span>
                                )}
                            </div>
