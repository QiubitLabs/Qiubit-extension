...
import { CloseIcon, CheckIcon, CopyIcon, QrCodeIcon, ChevronDownIcon } from '../../../shared/Icons';
...
function AddressSection({ label, address, logoUrl, hasCopied, onCopy, onShowQR }: AddressSectionProps) {
                    ...
                    >
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }} onClick={e => e.stopPropagation()}>
                                ...
                                >
                                    {hasCopied ? <CheckIcon size={14} className="text-success" /> : <CopyIcon size={14} />}
                                </button>
                                <button 
                                    className="action-btn-mini" 
...
export function AddressDrawer({ 
    ...
    const [activeQR, setActiveQR] = useState<{ address: string; label: string } | null>(null);
    ...
    const { copy: copyHyperliquid, hasCopied: hasCopiedHyperliquid } = useClipboard(2000, {
    ...
    const { copy: copyBitcoin, hasCopied: hasCopiedBitcoin } = useClipboard(2000, {
        onSuccess: () => showToast('Bitcoin address copied', 'success')
    });

    const handleCopy = (addr: string, type: 'octra' | 'evm' | 'solana' | 'sui' | 'bitcoin' | 'bsc' | 'monad' | 'hyperliquid') => {
            ...
            {isOpen && (
                <>
                    ...
                    >
                        ...
                        <div className="drawer-body" style={{ minHeight: '260px', paddingBottom: '16px' }}>
                        ...
                        <AnimatePresence>
                            {activeQR && (
                                ...
                                >
                                    <div className="qr-overlay-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                    ...
                                    <div className="qr-overlay-body" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                                        <div 
                                            style={{ 
                                                background: '#ffffff', 
                                                padding: '12px', 
                                        ...
                                        <div style={{ display: 'flex', gap: '12px', width: '100%', maxWidth: '320px', marginTop: 'auto' }}>
                                            <button 
                                                className="btn btn-secondary" 
                                                style={{ flex: 1, padding: '12px 0', borderRadius: '12px', fontSize: '0.875rem' }}
                                                onClick={() => {
                                                    if (activeQR.label.includes('Octra')) handleCopy(activeQR.address, 'octra');
                                                    else if (activeQR.label.includes('Ethereum') || activeQR.label.includes('EVM')) handleCopy(activeQR.address, 'evm');
                                                    else if (activeQR.label.includes('Binance') || activeQR.label.includes('BNB') || activeQR.label.includes('Smart Chain')) handleCopy(activeQR.address, 'bsc');
                                                    else if (activeQR.label.includes('Monad')) handleCopy(activeQR.address, 'monad');
                                                    else if (activeQR.label.includes('Hyperliquid')) handleCopy(activeQR.address, 'hyperliquid');
                                                    else if (activeQR.label.includes('Solana')) handleCopy(activeQR.address, 'solana');
                                                    else if (activeQR.label.includes('Sui')) handleCopy(activeQR.address, 'sui');
                                                    else if (activeQR.label.includes('Bitcoin')) handleCopy(activeQR.address, 'bitcoin');
                                                }}
                                            >
                                                Copy Address
                                            </button>
                                            <button 
                                                ...
                                                style={{ flex: 1, padding: '12px 0', borderRadius: '12px', fontSize: '0.875rem', background: 'var(--accent-color)' }}
                                                onClick={() => setActiveQR(null)}
                                            >
                                                Back
                                            </button>
