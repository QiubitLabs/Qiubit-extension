 ...
 */
export function OCS01TokenList({
  walletAddress,
  network = "testnet",
  hideBalance = false,
}: OCS01TokenListProps) {
  ...
  async function loadTokenBalances() {
    ...
    setIsLoading(true);
    try {
      const balances = await ocs01Manager.getUserTokenBalances(
        walletAddress,
        network,
      );
 ...
 */
export function OCS01ContractExplorer({
  walletAddress,
  network = "testnet",
}: OCS01ContractExplorerProps) {
    ...
    <div className="ocs01-explorer">
      ...
      {contracts.length > 0 ? (
        <>
          ...
          {methodResult && (
            <div
              className={`method-result ${methodResult.success ? "success" : "error"}`}
            >
              <span className="result-label">Result:</span>
              <span className="result-value">
                {methodResult.success
                  ? String(methodResult.result)
                  : methodResult.error}
              </span>
            </div>
          )}
