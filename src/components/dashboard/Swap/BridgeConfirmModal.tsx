  ...
  return (
    <div style={{ marginBottom: "12px" }}>
      ...
      >
        ...
        <button onClick={() => setOpen((v) => !v)} className="addr-chip-btn">
      ...
      {open && <div className="expanded-address-container active">{addr}</div>}
    </div>
  );
}
...
export function BridgeConfirmModal({
  ...
  const fromAddr =
    bridgeType === "burn" || bridgeType === "claim"
      ? wallet.evmAddress
      : address;
  ...
  const isInsufficientEth =
    ...
    ethBalanceWei !== null &&
    ethBalanceWei <
      claimGasOpts[claimFeeSpeed === "custom" ? "normal" : claimFeeSpeed]
        .maxFeePerGas *
        claimGasOpts.gasLimit;
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <span className="view-title">Confirm {actionLabel}</span>
      </div>
      ...
      <div className="details-card" style={{ marginBottom: "16px" }}>
        ...
        {isInsufficientEth && (
          <div className="warning-card-box">
            Insufficient ETH for gas fee. Please deposit more ETH to claim wOCT
            on Ethereum.
          </div>
        )}
        ...
        <div
          onClick={() =>
            isClaim || bridgeType === "burn"
              ? setShowClaimFeePopup(true)
              : setShowOctFeePopup(true)
          }
                ...
                {claimGasOpts && ethPriceUsd && (
                  <div className="network-fee-fiat">
                    ...
                    {(
                      (Number(
                        claimGasOpts[
                          claimFeeSpeed === "custom" ? "normal" : claimFeeSpeed
                        ].maxFeePerGas * claimGasOpts.gasLimit,
                      ) /
                        1e18) *
                      ethPriceUsd
                    ).toFixed(2)}
      ...
      {/* Simulation Card */}
      {bridgeError && <div className="warning-card-box">{bridgeError}</div>}
      ...
      <NetworkFeeSheet
        ...
        feeEstimates={{
          low: claimGasOpts
            ? Number(claimGasOpts.slow.maxFeePerGas * claimGasOpts.gasLimit) /
              1e18
            : 0,
          medium: claimGasOpts
            ? Number(claimGasOpts.normal.maxFeePerGas * claimGasOpts.gasLimit) /
              1e18
            : 0,
          high: claimGasOpts
            ? Number(claimGasOpts.fast.maxFeePerGas * claimGasOpts.gasLimit) /
              1e18
            : 0,
        }}
