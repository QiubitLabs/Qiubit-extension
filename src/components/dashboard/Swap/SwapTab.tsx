...
import { getMultipleTokenPrices, seedTokenPrice } from '../../../services/network/PriceService';
import { addCustomToken, fetchTokenMetadata } from '../../../services/features/CustomTokenService';
...
// Supported networks derived dynamically from the central registry
...
// Stable chain references — never use array indices (order depends on registry insertion order)
const ETH_CHAIN = EVM_SWAP_CHAINS.find(c => c.chainId === 1) ?? EVM_SWAP_CHAINS[0];
const BASE_CHAIN = EVM_SWAP_CHAINS.find(c => c.chainId === 8453) ?? ETH_CHAIN;


// Horizontal list of networks for selection tab derived dynamically from registry
const HORIZONTAL_NETWORKS: { id: string; name: string; logoUrl: string; chainId?: number }[] = [
...
export interface SwapTabProps {
    ...
    setSwapStep?: (step: 'form' | 'submitting' | 'waiting' | 'success' | 'failed') => void;
...
export function SwapTab({
    ...
    const [fromToken, setFromToken] = useState<any>({ ...DEFAULT_TOKENS[1][0], chainId: 1 }); // ETH
    const [toToken, setToToken] = useState<any>({ ...DEFAULT_TOKENS[8453][0], chainId: 8453 }); // ETH on Base
    ...
    // Dynamic EVM Swapping state
    const [quote, setQuote] = useState<any>(null);
    const [isFetchingQuote, setIsFetchingQuote] = useState(false);
    const [quoteError, setQuoteError] = useState('');

    const [localSwapStep, setLocalSwapStep] = useState<'form' | 'submitting' | 'waiting' | 'success' | 'failed'>('form');
    const swapStep = propSwapStep !== undefined ? propSwapStep : localSwapStep;
    const setSwapStep = propSetSwapStep !== undefined ? propSetSwapStep : setLocalSwapStep;
    const [execStatus, setExecStatus] = useState('');
    const [execError, setExecError] = useState('');
    const [txHash, setTxHash] = useState('');
    const [lifiStatus, setLifiStatus] = useState<string>('');

    // Confirmation & Gas fee selection states
    const [localShowConfirm, setLocalShowConfirm] = useState(false);
    const showConfirmModal = propShowConfirmModal !== undefined ? propShowConfirmModal : localShowConfirm;
    const setShowConfirmModal = propSetShowConfirmModal !== undefined ? propSetShowConfirmModal : setLocalShowConfirm;
    const [isEstimatingGas, setIsEstimatingGas] = useState(false);
    const [swapFeeSpeed, setSwapFeeSpeed] = useState<'slow' | 'normal' | 'fast' | 'custom'>('normal');
    ...
    const [swapFeeEstimates, setSwapFeeEstimates] = useState({ low: 0.001, medium: 0.005, high: 0.01 });

    // Dynamic Token Selector modal filters
    ...
    // Custom token resolution — resolves metadata when user pastes a contract address
    ...
    // Quote request tracking — prevents stale async responses from overwriting newer state
    const quoteRequestId = useRef(0);

    // When Octra network is selected, redirect to OctraBridgeView instead of inline
    ...
    useEffect(() => {
        if (isOctraBridgeActive) {
            // Reset chains back to EVM defaults and navigate to Octra Bridge
    ...
    // Resolve contract address typed/pasted into the token search box.
    // Priority: LI.FI token API (gives logo URL + price) → on-chain RPC fallback.
    // Resolved token is marked _imported=true so selectTokenAndChain knows to persist it.
    useEffect(() => {
        ...
        const netDef = HORIZONTAL_NETWORKS.find(n => n.id === activeChainFilter && n.id !== 'all');
        const chainId = netDef?.chainId ?? (showTokenSelector === 'to' ? toChain.chainId : fromChain.chainId);
        ...
        fetch(`https://li.quest/v1/token?chain=${chainId}&token=${query}`, { headers })
            .then(async r => {
            ...
            .catch(async () => {
                if (callId !== resolveRef.current) return;
                // Fallback: fetch metadata directly from the chain RPC
                try {
                    const meta = await fetchTokenMetadata(query, chainId);
                    if (callId === resolveRef.current) {
                        setCustomToken({ ...meta, balance: '0', priceUSD: null, _imported: true });
                    }
                } catch {
    ...
    }, [searchQuery, activeChainFilter, showTokenSelector, fromChain.chainId, toChain.chainId]);

    // Fetch prices only for selected tokens and their chain native symbols to avoid rate limiting
    useEffect(() => {
    ...
    // Pre-calculate user balances
    const userBalance = useMemo(() => {
        if (fromChain.id === 'octra') {
            return octBalance || 0;
        }
        if (!allTokens) return 0;
        const matching = allTokens.find(t =>
            t.chainId === fromChain.chainId &&
            t.symbol.toUpperCase() === fromToken.symbol.toUpperCase()
        );
        if (!matching) return 0;
        return typeof matching.balance === 'string' ? parseFloat(matching.balance) : (matching.balance || 0);
    ...
    const fromUsdValue = useMemo(() => {
        // Use LI.FI quote's fromAmountUSD when available (real data)
        if (quote?.estimate?.fromAmountUSD) return parseFloat(quote.estimate.fromAmountUSD);
    ...
    const toUsdValue = useMemo(() => {
        // Use LI.FI quote's toAmountUSD when available (real data)
        if (quote?.estimate?.toAmountUSD) return parseFloat(quote.estimate.toAmountUSD);
    ...
    // Enforce same-chain in Swap mode; fix duplicate token selection.
    useEffect(() => {
        if (activeTab !== 'swap') return;
        if (fromChain.id !== toChain.id) {
            setToChain(fromChain);
            const defaults = DEFAULT_TOKENS[fromChain.chainId] || [];
            const nextTok = defaults.find(d => d.symbol !== fromToken.symbol) || defaults[0];
            if (nextTok) setToToken({ ...nextTok, chainId: fromChain.chainId });
        } else if (fromToken.contractAddress === toToken.contractAddress && fromToken.chainId === toToken.chainId) {
            const defaults = DEFAULT_TOKENS[fromChain.chainId] || [];
            const nextTok = defaults.find(d => d.contractAddress !== fromToken.contractAddress) || defaults[1] || defaults[0];
            if (nextTok && nextTok.contractAddress !== fromToken.contractAddress) {
                setToToken({ ...nextTok, chainId: fromChain.chainId });
            }
        }
    }, [activeTab, fromChain, fromToken, toToken.contractAddress, toToken.chainId]);

    // Handle swap mode tab switching cleanup
    const handleTabChange = (tab: 'swap' | 'bridge') => {
        ...
        if (tab === 'swap') {
            ...
            // Bridge tab: if both chains are the same (locked by swap mode), pick a cross-chain destination
            if (fromChain.chainId === toChain.chainId) {
                ...
                if (toNative) setToToken({ ...toNative, chainId: defaultToChain.chainId });
            }
        }
    };

    // Set from input amount to maximum balance with smart dynamic gas fee simulation
    const handleMaxInput = () => {
        if (userBalance > 0) {
            // Only deduct if the token is the native gas token of the chain
            const isNative = fromChain.id === 'octra' || 
                (fromToken && (
            ...
            if (!isNative) {
                // Non-native assets (like stablecoins or custom tokens) do not pay gas in their own currency
                setFromAmount(parseFloat(userBalance.toFixed(6)).toString());
                return;
            }

            // Native assets require dynamic gas simulation/estimation
            if (fromChain.id === 'octra') {
                try {
                    const rpcClient = getRpcClient();
                    rpcClient.getFeeEstimate(userBalance)
                        .then(fees => {
            ...
            } else if (fromChain.id !== 'solana' && fromChain.id !== 'sui' && fromChain.id !== 'bitcoin') {
                // EVM networks: fetch live gas fee estimation from the network provider
                ...
                fetchGasOptions({}, fallbackLimit, fromChain.id)
                    .then(opts => {
                        ...
                        const dynamicFeeEth = Number(feeTier.maxFeePerGas * opts.gasLimit) / 1e18;
                        const maxAmt = Math.max(0, userBalance - dynamicFeeEth);
                        setFromAmount(parseFloat(maxAmt.toFixed(6)).toString());
                    })
                    .catch(err => {
                        console.error('Failed to simulate EVM max gas:', err);
                        // Safe fallback buffer in case of network timeout/offline state
                ...
                // Non-EVM fallbacks (Solana, Sui, Bitcoin)
    ...
    // Select token and chain via the unified Token Selector Modal.
    // If the token was resolved from a pasted contract address (_imported), persist it to
    // the user's custom token list so it shows in the Home page with balance and price.
    const selectTokenAndChain = (token: any, target: 'from' | 'to') => {
        // wOCT selected anywhere → route to OCT↔wOCT bridge
        const isWOCT = token.symbol?.toLowerCase() === 'woct' ||
            token.contractAddress?.toLowerCase() === '0x4647e1fe715c9e23959022c2416c71867f5a6e80';
        if (isWOCT) {
        ...
        if (token._imported && wallet.address && token.contractAddress !== '0x0000000000000000000000000000000000000000') {
            const checksummed = ethers.getAddress(token.contractAddress);
            // Seed price cache immediately so Home page shows price without waiting for CoinGecko
        ...
        const targetChain = EVM_SWAP_CHAINS.find(c => c.chainId === token.chainId) ?? ETH_CHAIN;
    ...
    // Fetch quote dynamically on input changes
    const getLiFiQuote = async () => {
        // Same chain, same token — no swap needed
        if (fromChain.chainId === toChain.chainId &&
            fromToken.contractAddress === toToken.contractAddress) {
            ++quoteRequestId.current;
            setQuote(null);
            setToAmount('');
            setQuoteError('Same token selected — please choose a different token to swap.');
            return;
        }
        ...
        if (!fromAmount || parseFloat(fromAmount) <= 0 || !fromAddr || !toAddr) {
            // Cancel any in-flight request so its stale result won't apply
            ++quoteRequestId.current;
            setQuote(null);
            setToAmount('');
            setQuoteError('');
        ...
        try {
            ...
            const rawAmt = ethers.parseUnits(sanitizedAmount, fromToken.decimals).toString();
            ...
            const lifiIntegrator = (import.meta.env.VITE_LIFI_INTEGRATOR_ID as string) || '';
            const lifiFeeBps = parseInt((import.meta.env.VITE_LIFI_FEE_BPS as string) || '0', 10);
            if (lifiIntegrator) params.set('integrator', lifiIntegrator);
            if (lifiIntegrator && lifiFeeBps > 0) params.set('fee', (lifiFeeBps / 10000).toFixed(4));

            const lifiApiKey = (import.meta.env.VITE_LIFI_API_KEY as string) || '';
            const headers: HeadersInit = lifiApiKey ? { 'x-lifi-api-key': lifiApiKey } : {};
            ...
            // Use decimals from LI.FI's response — authoritative for the actual output token
            ...
            setQuote(data);
            setToAmount(Number(ethers.formatUnits(data.estimate.toAmount, resolvedToDecimals)).toFixed(6));
        } catch (e: any) {
            if (requestId !== quoteRequestId.current) return;
    ...
    const handleSwapClick = async () => {
        ...
        if (fromChain.id === 'solana' || fromChain.id === 'sui' || fromChain.id === 'bitcoin') {
        ...
        try {
            ...
            console.error('Failed to estimate gas for swap:', e);
    ...
    // Native Cross-Chain / Swap Transaction Execution
    const executeSwap = async () => {
        ...
        try {
            if (fromChain.id === 'sui' || fromChain.id === 'bitcoin') {
                throw new Error(`Cross-chain swaps starting from ${fromChain.name} are currently supported in receive mode. Please swap from an EVM network or Solana to receive ${fromChain.name} assets.`);
            }

            if (fromChain.id === 'solana') {
                setExecStatus('Submitting swap to Solana network...');
                const txData = quote.transactionRequest?.data;
                if (!txData) throw new Error('No transaction request returned from LI.FI.');
                ...
                setTxHash(signature);
                setSwapStep('waiting');
                setExecStatus('Solana transaction submitted! Tracking cross-chain execution...');
                ...
                for (let i = 0; i < 60; i++) {
                    ...
                    try {
                        ...
                        const statusResp = await fetch(`https://li.quest/v1/status?txHash=${signature}&fromChain=${fromChain.chainId}&toChain=${toChain.chainId}`,
                        ...
                        // Handled gracefully on retry
                    }
                }

                if (!completed) {
                    setExecStatus('Transaction executing safely on Solana. Please verify inside Solscan.');
                    setSwapStep('success');
                }
                return;
            }
            ...
            const rpcUrl = NETWORK_REGISTRY[evmNetworkKey]?.rpcUrl;
            if (!rpcUrl) throw new Error(`RPC endpoint missing for ${fromChain.name}`);

            if (fromToken.contractAddress !== '0x0000000000000000000000000000000000000000' && quote.estimate.approvalAddress) {
                const spender = quote.estimate.approvalAddress;
                setExecStatus('Checking token spend allowance...');

                const provider = new ethers.JsonRpcProvider(rpcUrl);
                const erc20 = new ethers.Contract(fromToken.contractAddress, [
                    'function allowance(address owner, address spender) view returns (uint256)'
                ], provider);

                const currentAllowance = await erc20.allowance(wallet.evmAddress, spender);
                let sanitizedAmount = fromAmount.replace(/,/g, '.');
                const parts = sanitizedAmount.split('.');
                if (parts[1] && parts[1].length > fromToken.decimals) {
                    sanitizedAmount = `${parts[0]}.${parts[1].substring(0, fromToken.decimals)}`;
                }
                const rawAmount = ethers.parseUnits(sanitizedAmount, fromToken.decimals);
            ...
            const gasLimitOverride = swapGasOpts?.gasLimit ? BigInt(swapGasOpts.gasLimit) : (txReq.gasLimit ? BigInt(txReq.gasLimit) : undefined);
            ...
            if (swapGasOpts) {
                if (swapFeeSpeed === 'custom') {
                    ...
                    txOverride.maxPriorityFeePerGas = txOverride.maxFeePerGas > 1_000_000_000n ? 1_000_000_000n : txOverride.maxFeePerGas;
                } else {
            ...
            const tx = await keyringService.signAndSendEvm(address, txOverride, rpcUrl);
            ...
            for (let i = 0; i < 60; i++) {
                ...
                try {
                    const _lifiKey = (import.meta.env.VITE_LIFI_API_KEY as string) || '';
                    const statusResp = await fetch(`https://li.quest/v1/status?txHash=${tx.hash}&fromChain=${fromChain.chainId}&toChain=${toChain.chainId}`,
                        _lifiKey ? { headers: { 'x-lifi-api-key': _lifiKey } } : {}
                    );
                    const statusData = await statusResp.json();
                    ...
                    if (stat === 'DONE') {
                        ...
                        completed = true;
                        onRefresh();
                        // Persist swap tx so it appears in history immediately
                        if (wallet?.evmAddress) {
                            const netId = Object.values(NETWORK_REGISTRY).find(n => n.chainId === fromChain.chainId)?.id ?? 'ethereum';
                            const toNetId = Object.values(NETWORK_REGISTRY).find(n => n.chainId === toChain.chainId)?.id;
                            const swapTx = {
                                hash: tx.hash,
                                type: 'swap' as const,
                            ...
                            if (wallet.address) {
                                saveTxHistorySecure([{ ...swapTx, description: 'Swap' }], 'mainnet', wallet.address).catch(() => {});
                            }
                        }
                        break;
                    } else if (stat === 'FAILED') {
            ...
            if (!completed) {
                setExecStatus('Transaction executing safely on-chain. Please verify inside explorer.');
                setSwapStep('success');
                // Still save tx even if status poll timed out
                if (wallet?.evmAddress) {
                    const netId = Object.values(NETWORK_REGISTRY).find(n => n.chainId === fromChain.chainId)?.id ?? 'ethereum';
                    const toNetId = Object.values(NETWORK_REGISTRY).find(n => n.chainId === toChain.chainId)?.id;
                    const swapTx = {
            ...
            setExecError(e.message || 'Swap execution failed.');
            setSwapStep('failed');
        }
    };
    ...
    return (
        <>
            {swapStep === 'form' && (
                showConfirmModal ? (
                    <SwapConfirmModal
                        ...
                        fee={
                            swapFeeSpeed === 'custom'
                            ? (swapGasOpts ? Number(gweiToWei(customSwapFeeGwei) * swapGasOpts.gasLimit) / 1e18 : swapFeeEstimates.medium)
                            : (swapFeeSpeed === 'slow' ? swapFeeEstimates.low : swapFeeSpeed === 'fast' ? swapFeeEstimates.high : swapFeeEstimates.medium)
                        }
                        ethPriceUsd={tokenPrices.get(fromChain.symbol.toUpperCase())?.price || null}
                    ...
                    <div className="swap-widget-card">
                        ...
                        <div className="widget-section">
                            <div className="section-header">
                                <div className="section-label-group">
                                    <span>Dari</span>
                                    <img src={fromChain.logoUrl} alt="" className="inline-chain-icon" />
                                    <span className="chain-name-text">{fromChain.name}</span>
                                </div>
                                <div className="section-balance-group" onClick={handleMaxInput}>
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="wallet-svg-icon">
                                        <rect x="2" y="5" width="20" height="14" rx="2" ry="2"></rect>
                                        <line x1="2" y1="10" x2="22" y2="10"></line>
                                    </svg>
                                    <span className="balance-num">{formatAmount(userBalance, 6)}</span>
                            ...
                            <div className="token-input-row">
                                ...
                                >
                                    <TokenIcon symbol={fromToken.symbol} logoUrl={fromToken.logoUrl} size={24} />
                                    <span className="token-symbol-text">{fromToken.symbol}</span>
                                ...
                                <div className="amount-input-container">
                                    ...
                                    <div className="amount-fiat-val">
                                        {fromUsdValue !== null ? `$${fromUsdValue.toFixed(2)}` : '$0.00'}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="widget-section">
                            <div className="section-header">
                                <div className="section-label-group">
                                    <span>Ke</span>
                                    <img src={toChain.logoUrl} alt="" className="inline-chain-icon" />
                                ...
                                <div className="section-balance-group">
                                    ...
                                    <span className="balance-num">{formatAmount(toTokenBalance, 6)}</span>
                                </div>
                            </div>
                            <div className="token-input-row">
                                <div
                                ...
                                >
                                    <TokenIcon symbol={toToken.symbol} logoUrl={toToken.logoUrl} size={24} />
                                    ...
                                    <div className="amount-fiat-val">
                                        {toUsdValue !== null ? (
                                            <>
                                                ...
                                                {priceImpactText && <span className="price-impact-tag">{priceImpactText}</span>}
                                            </>
                                        ) : '$0.00'}
                                    </div>
                                </div>
                    ...
                    {quoteError && (
                        <div className="bridge-notice" style={{ background: 'rgba(239, 68, 68, 0.04)', borderColor: 'rgba(239, 68, 68, 0.12)' }}>
                            <span className="notice-text" style={{ color: '#f87171' }}>{quoteError}</span>
                        </div>
                    )}
                        ...
                        <div className="widget-route-card animate-fade-in">
                            ...
                            <div className="info-row" style={{ marginBottom: '6px' }}>
                                ...
                                <span className="info-value">
                                    ~{Math.round((quote.estimate?.executionDuration || 120) / 60)} minutes
                                </span>
                            </div>
                            <div className="info-row">
                                <span className="info-label">Estimated Gas</span>
                                <span className="info-value">
                                    ${Number(quote.estimate?.feeUsd || 0).toFixed(2)} USD
                                </span>
                            </div>
                        </div>
                    )}
            ...
            {(swapStep === 'submitting' || swapStep === 'waiting' || swapStep === 'success' || swapStep === 'failed') && (
                <SwapStatusOverlay
