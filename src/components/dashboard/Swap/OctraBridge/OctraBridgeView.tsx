...
import { BRIDGE_VAULT, WOCT_ADDR, ETH_BRIDGE, RECOVERY_URL, signerPost } from '../../../../utils/octra/bridge';
...
export function OctraBridgeView({ wallet, address, balance, onRefresh }: OctraBridgeViewProps) {
    ...
    const [completedInfo, setCompletedInfo] = useState<{ amount: string; dir: 'o2e' | 'e2o'; status: string } | null>(null);
    ...
    async function fetchUnclaimedProofs() {
        ...
        const candidates: Array<{ epochId: number; leafIndex: number; amount: string; calldata?: string }> = [];
        ...
        try {
            ...
            if (resp.ok) {
                ...
                const matchedKey = Object.keys(by).find(k => k.toLowerCase() === target);
                ...
                if (Array.isArray(bucket)) {
                    bucket.forEach(m => {
                        ...
                        const amount    = (Number(String(m.amount_raw || '0')) / 1e6).toString();
                        if (epochId && amount !== '0') candidates.push({ epochId, leafIndex, amount });
                    });
                }
            }
        } catch (e) { console.warn('Failed to fetch from recovery.json:', e); }
        ...
        claimKeys.forEach(key => {
            ...
            } catch (e) { console.warn(`Failed to parse pending claim for key ${key}`, e); }
        });
        ...
        epochKeys.forEach(key => {
            ...
            try {
                ...
                const amount    = le.amount || le.amt_display || (le.amount_raw ? (Number(le.amount_raw) / 1e6).toString() : '0');
                const leafIndex = le.leafIndex || le.leaf_index || 0;
                if (epochId && !candidates.some(c => c.epochId === epochId)) {
                    candidates.push({ epochId, leafIndex, amount });
                }
            } catch (e) { console.warn(`Failed to parse pending epoch for key ${key}`, e); }
        });
                ...
                if (Array.isArray(historyArr)) {
                    historyArr.forEach(h => {
                        if (h && h.status !== 'claimed' && h.status !== 'unlocked') {
                            const epochId   = typeof h.epoch === 'number' ? h.epoch : parseInt(h.epoch, 10);
                            const rawAmt    = h.amount_raw ? String(h.amount_raw) : '0';
                            const amtDisplay = h.amt_display || (Number(rawAmt) / 1e6).toString();
                            const leafIndex = h.leaf_index || 0;
                            const calldata  = h.claim_data?.calldata;
                            if (epochId && !candidates.some(c => c.epochId === epochId)) {
                                candidates.push({ epochId, leafIndex, amount: amtDisplay, calldata });
                            }
                        }
                    });
                }
            } catch (e) { console.warn('Failed to parse bridge_history from localStorage', e); }
        }
            ...
            const checkedProofs = await Promise.all(
                candidates.map(async (c) => {
                    try {
                        ...
                        if (!calldata) {
                            try {
                                const msgData  = await signerPost('bridgeMessagesByEpoch', [c.epochId]);
                                const messages = msgData.result?.messages || [];
                                const myMsg    = messages.find((m: any) => m.recipient?.toLowerCase() === target);
                                if (myMsg) {
                                    leafIndex = myMsg.leaf_index;
                                    const cdData = await signerPost('bridgeClaimCalldata', [c.epochId, leafIndex]);
                                    if (cdData.result?.calldata) calldata = cdData.result.calldata;
                                } else {
                                    const cdData = await signerPost('bridgeClaimCalldata', [c.epochId, leafIndex || 0]);
                                    if (cdData.result?.calldata) calldata = cdData.result.calldata;
                                }
                            } catch {
                                try {
                        ...
                        try {
                            ...
                            await provider.call({ to: ETH_BRIDGE, data: calldata!, from: ethRecipient });
                            return { calldata, epochId: c.epochId, amount: c.amount, leafIndex: leafIndex || c.leafIndex };
                        } catch (simErr: any) {
                            const simMsg = (simErr?.message || simErr?.data || '').toLowerCase();
                            if (simMsg.includes('already') || simMsg.includes('replay') || simErr?.data === '0xb5a78004') {
                                return null;
                            }
                            return { calldata, epochId: c.epochId, amount: c.amount, leafIndex: leafIndex || c.leafIndex };
                        }
                        ...
                        if (msg.includes('already') || msg.includes('replay') || err?.data === '0xb5a78004') return null;
                        return null;
                    }
                })
            );

            const activeProofs = (checkedProofs.filter(p => p !== null) as any[])
                .sort((a, b) => b.epochId - a.epochId);

            setUnclaimedProofs(activeProofs);
            if (activeProofs.length > 0 && !pendingClaim) setPendingClaim(activeProofs[0]);
        } catch (e) { console.warn('Failed to resolve proofs:', e); }
        finally { setIsFetchingUnclaimed(false); }
    }
    ...
    // Resume epoch polling on mount if there's a pending epoch
    useEffect(() => {
    ...
    async function waitForOctraReceipt(txHash: string, maxSec = 60): Promise<any> {
    ...
    async function waitForClaimData(epochId: number, recipient: string): Promise<{ calldata: string; epochId: number } | null> {
        const start = Date.now();
        let attempt = 0;
            ...
            try {
                ...
                if (headerData.result && headerData.result.message_count > 0) {
                    ...
                    if (cdData.result?.calldata) {
                        try {
                            const provider = new ethers.JsonRpcProvider(getEvmRpcUrl());
                            await provider.call({ to: ETH_BRIDGE, data: cdData.result.calldata });
    ...
    const lockOctToEth = async () => {
        ...
        if (!recip || !/^0x[0-9a-fA-F]{40}$/.test(recip)) { setBridgeError('Enter a valid Ethereum address'); return; }
        if (!fromAmount || parseFloat(fromAmount) <= 0)     { setBridgeError('Enter amount'); return; }

        setBridgeStep(1); setBridgeError(''); setBridgeStatus('Locking OCT on Octra...'); setLockTxHash('');
        ...
        try {
            ...
            const signedTx = await createTransaction(
                address, BRIDGE_VAULT, fromAmount, nonce + 1, privateKeyB64,
                JSON.stringify([recip]), selectedOctFee.toFixed(4), 'call', 'lock_to_eth'
            );
    ...
    const openClaimConfirm = async (claimOverride?: any) => {
        ...
        try {
            const [opts, priceData, balWei] = await Promise.all([
                fetchGasOptions({ to: ETH_BRIDGE, data: activeClaim.calldata, from: wallet.evmAddress }, 400_000n),
                getTokenPrice('ETH'),
                wallet.evmAddress
                    ? new ethers.JsonRpcProvider(getEvmRpcUrl()).getBalance(wallet.evmAddress!).catch(() => null)
        ...
        } catch { setClaimGasOpts(null); }
        finally { setIsFetchingClaimFee(false); }
    };
    ...
    const claimWoct = async () => {
        ...
        try {
            const gasOpts0   = claimGasOpts ?? await fetchGasOptions({ to: ETH_BRIDGE, data: pendingClaim.calldata, from: wallet.evmAddress }, 400_000n);
            const requiredWei = gasOpts0.gasLimit * gasOpts0[claimFeeSpeed === 'custom' ? 'normal' : claimFeeSpeed].maxFeePerGas;
            const balanceWei  = ethBalanceWei ?? (wallet.evmAddress
                ? await new ethers.JsonRpcProvider(getEvmRpcUrl()).getBalance(wallet.evmAddress!).then(b => BigInt(b.toString())).catch(() => null)
                : null);
            ...
            if (balanceWei !== null && balanceWei < requiredWei) {
                ...
                setBridgeError(`Insufficient ETH: you have ${hasEth} ETH but need at least ${reqEth} ETH for gas.`);
            ...
            try {
                ...
                const isAlreadyClaimed = msg.includes('0xb5a78004') || msgLower.includes('already') || msgLower.includes('replay');
                if (isAlreadyClaimed) {
                    setShowClaimConfirm(false);
                    setCompletedInfo({ amount: pendingClaim.amount || fromAmount, dir: 'o2e', status: 'wOCT already claimed!' });
                ...
                if (msgLower.includes('insufficient funds')) {
                    if (balanceWei !== null && balanceWei < requiredWei) {
                        const reqEth = (Number(requiredWei) / 1e18).toFixed(6);
                        setBridgeError(`Insufficient ETH for gas fee. You need at least ${reqEth} ETH.`);
                        setIsFetchingClaimFee(false);
                        return;
                    }
                } else {
            ...
            const gasOpts = claimGasOpts ?? await fetchGasOptions({ to: ETH_BRIDGE, data: pendingClaim.calldata, from: wallet.evmAddress }, 400_000n);
            ...
            setCompletedInfo({ amount: pendingClaim.amount || fromAmount, dir: 'o2e', status: 'wOCT claimed successfully!' });
            setBridgeStep(4);
    ...
    const burnWoctToOct = async (overrideGasOpts?: any, overrideFeeSpeed?: string, overrideCustomGwei?: string) => {
        const recip = octRecipient.trim();
        if (!recip || recip.length !== 47 || !recip.startsWith('oct')) { setBridgeError('Octra address missing in wallet'); return; }
        if (!fromAmount || parseFloat(fromAmount) <= 0)                 { setBridgeError('Enter amount'); return; }
        ...
        try {
            ...
            const selectedGasOpts = overrideGasOpts || claimGasOpts || await fetchGasOptions({ to: ETH_BRIDGE, from: wallet.evmAddress }, 180_000n);
            ...
            const approveTx = await keyringService.signAndSendEvm(address, {
                to: WOCT_ADDR, data: approveData, gasLimit: 0x30000n, maxFeePerGas, maxPriorityFeePerGas
            }, getEvmRpcUrl());
            ...
            const burnTx = await keyringService.signAndSendEvm(address, {
                to: ETH_BRIDGE, data: burnData, gasLimit: 0x40000n, maxFeePerGas, maxPriorityFeePerGas
            }, getEvmRpcUrl());
            ...
            const burnStatus = unlocked ? 'OCT unlocked successfully!' : 'wOCT burned. OCT will arrive on Octra in ~2 min.';
        ...
        if (bridgeDir === 'e2o') {
            ...
            try {
                ...
                const [opts, priceData] = await Promise.all([
                    fetchGasOptions({ to: ETH_BRIDGE, from: wallet.evmAddress, data: burnData }, 180_000n),
                    getTokenPrice('ETH'),
                ]);
                setClaimGasOpts(opts);
                setCustomClaimGasPriceGwei((Number(opts.normal.maxFeePerGas) / 1e9).toFixed(2));
    ...
    const resetBridge = () => {
        setBridgeStep(0); setBridgeStatus(''); setBridgeError('');
        setPendingClaim(null); setPendingEpoch(null);
        setIsClaimSubmitting(false); setCompletedInfo(null);
        setFromAmount(''); setLockTxHash('');
    };
    ...
    return (
        <>
            ...
            {bridgeStep > 0 && bridgeStep < 4 && (
                <div className="full-page-overlay animate-fade-in" style={{ padding: '16px', background: '#0D0D0D', zIndex: 1500 }}>
                    <div className="complete-card animate-fade-in">
                        ...
                        <div className="progress-steps-list">
                            <div className={`progress-step-item ${bridgeStep >= 1 ? 'completed' : ''}`}>
                                <div className="step-icon-wrap">
                                    {bridgeStep > 1 ? <CheckIcon size={14} /> : <div className="spinner-small" />}
                                </div>
                                <div className="step-text-label">
                                    {bridgeDir === 'o2e' ? 'Locking OCT on Octra' : 'Approving and Burning wOCT'}
                                </div>
                            </div>
                            <div className="step-connector-line" />
                            <div className={`progress-step-item ${bridgeStep >= 2 ? (bridgeStep > 2 ? 'completed' : 'active') : ''}`}>
                                <div className="step-icon-wrap">
                                    {bridgeStep > 2 ? <CheckIcon size={14} /> : bridgeStep === 2 ? <div className="spinner-small" /> : <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'currentColor' }} />}
                                </div>
                                <div className="step-text-label">
                                    {bridgeDir === 'o2e' ? 'Waiting for Bridge Header' : 'Decrypting and Unlocking OCT'}
                                </div>
                            </div>
                            <div className="step-connector-line" />
                            <div className={`progress-step-item ${bridgeStep === 3 ? 'active' : ''}`}>
                                <div className="step-icon-wrap">
                ...
                <div className="full-page-overlay animate-fade-in" style={{ padding: '16px', background: '#0D0D0D', zIndex: 1500 }}>
                    <div className="complete-card animate-fade-in">
                        <div className="success-icon-wrap">
                            <div className="success-icon-circle"><CheckIcon size={24} /></div>
                        </div>
                        <div className="complete-title">Bridge Complete</div>
                        <div className="success-amount-display">
                            {completedInfo.amount} {completedInfo.dir === 'o2e' ? 'wOCT' : 'OCT'}
                        </div>
                        <div className="success-route-container">
                            <div className="success-chain-item">
                                <img src={completedInfo.dir === 'o2e' ? '/qiubit-icon.svg' : '/eth-icon.svg'} alt="" style={{ width: 14, height: 14, borderRadius: '50%' }} />
                                <span>{completedInfo.dir === 'o2e' ? 'Octra' : 'Ethereum'}</span>
                            </div>
                            <div className="success-arrow-label">to</div>
                            <div className="success-chain-item">
                                <img src={completedInfo.dir === 'o2e' ? '/eth-icon.svg' : '/qiubit-icon.svg'} alt="" style={{ width: 14, height: 14, borderRadius: '50%' }} />
                                <span>{completedInfo.dir === 'o2e' ? 'Ethereum' : 'Octra'}</span>
                            </div>
                        </div>
                        {completedInfo.dir === 'e2o' && !completedInfo.status.includes('successfully') && (
                            <div className="progress-notice-card" style={{ marginBottom: '16px' }}>
                                <div className="notice-text-content">{completedInfo.status}</div>
                            </div>
                        )}
                        <button className="done-action-btn" onClick={resetBridge}>Done</button>
                    </div>
                </div>
            )}
            ...
            {bridgeError && bridgeStep === 0 && (
                <div className="full-page-overlay animate-fade-in" style={{ padding: '16px', background: '#0D0D0D', zIndex: 1500 }}>
                    <div className="complete-card animate-fade-in">
                        <div className="success-icon-wrap">
                            <div className="success-icon-circle" style={{ background: 'var(--accent-red, #ef4444)', boxShadow: '0px 8px 24px rgba(239, 68, 68, 0.1)' }}>
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                                    <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
                                </svg>
                            </div>
                        </div>
                        <div className="complete-title" style={{ color: 'var(--accent-red, #ef4444)' }}>Bridge Failed</div>
                        <div className="progress-notice-card" style={{ marginBottom: '16px', borderColor: 'rgba(239, 68, 68, 0.2)', background: 'rgba(239, 68, 68, 0.04)' }}>
                            <div className="notice-text-content" style={{ color: 'var(--accent-red, #f87171)' }}>{bridgeError}</div>
                        </div>
                        <button className="done-action-btn" onClick={resetBridge}>Try Again</button>
                    </div>
                </div>
            )}
            ...
            {bridgeStep === 0 && !completedInfo && !bridgeError && (
                <div className="octra-bridge-form animate-fade-in">
                    <div className="swap-widget-card">
                        ...
                        <div className="widget-section">
                            <div className="section-header">
                                <div className="section-label-group">
                                    <span>Dari</span>
                                    <img
                                        src={bridgeDir === 'o2e' ? '/qiubit-icon.svg' : '/eth-icon.svg'}
                                        alt=""
                                        className="inline-chain-icon"
                                    />
                                    <span className="chain-name-text">{bridgeDir === 'o2e' ? 'Octra' : 'Ethereum'}</span>
                                </div>
                                <div className="section-balance-group" onClick={() => setFromAmount(String(fromBalance))}>
                            ...
                            <div className="token-input-row">
                                <div className="token-dropdown-trigger borderless-trigger" style={{ cursor: 'default' }}>
                                    <img
                                        src={bridgeDir === 'o2e' ? '/qiubit-icon.svg' : '/chains/ethereum/woct.svg'}
                                        alt={fromSymbol}
                                        width={24}
                                        height={24}
                        ...
                        <div className="mid-swap-divider">
                            ...
                            <button
                                ...
                                onClick={() => { setBridgeDir(d => d === 'o2e' ? 'e2o' : 'o2e'); setFromAmount(''); }}
                            >
                        ...
                        <div className="widget-section">
                            <div className="section-header">
                                <div className="section-label-group">
                                    <span>Ke</span>
                                    <img
                                        src={bridgeDir === 'o2e' ? '/eth-icon.svg' : '/qiubit-icon.svg'}
                                        alt=""
                                        className="inline-chain-icon"
                                    />
                                    <span className="chain-name-text">{bridgeDir === 'o2e' ? 'Ethereum' : 'Octra'}</span>
                                </div>
                                <div className="section-balance-group">
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="wallet-svg-icon">
                                        <rect x="2" y="5" width="20" height="14" rx="2" ry="2" /><line x1="2" y1="10" x2="22" y2="10" />
                                    </svg>
                                    <span className="balance-num">0.000000</span>
                                </div>
                            </div>
                            <div className="token-input-row">
                                <div className="token-dropdown-trigger borderless-trigger" style={{ cursor: 'default' }}>
                                    <img
                                        src={bridgeDir === 'o2e' ? '/chains/ethereum/woct.svg' : '/qiubit-icon.svg'}
                                        alt={toSymbol}
                                        width={24}
                                        height={24}
                                        style={{ borderRadius: '50%', display: 'block' }}
                                ...
                                <div className="amount-input-container">
                                    <input
                                        ...
                                        readOnly
                                    />
                                    <div className="amount-fiat-val">1 {fromSymbol} = 1 {toSymbol}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {unclaimedProofs.length > 1 && (
                        <div className="unclaimed-proofs-section animate-fade-in">
                            <div className="unclaimed-proofs-title">
                                {isFetchingUnclaimed ? 'Checking unclaimed...' : `${unclaimedProofs.length} unclaimed bridge(s)`}
                            </div>
                            {unclaimedProofs.slice(0, 3).map(proof => (
                                <div key={proof.epochId} className="unclaimed-proof-item">
                                    <span>Epoch #{proof.epochId} — {proof.amount} wOCT</span>
                                    <button
                                        className="claim-action-btn-compact"
                    ...
                    >
                        {parseFloat(fromAmount) > fromBalance ? 'Insufficient Balance' : 'Bridge Assets'}
                    </button>
                </div>
            )}
        </>
