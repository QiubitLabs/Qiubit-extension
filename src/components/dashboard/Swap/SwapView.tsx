...
// ─── Native Bridge Registry ────────────────────────────────────────────────────
// Each entry maps a bridge key to its component.
// To add a new chain-native bridge (e.g. TON↔wTON):
//   1. Create src/components/dashboard/Swap/TonBridge/TonBridgeView.tsx
//   2. Add an entry: ton: { label: 'TON Bridge (TON ↔ wTON)', component: TonBridgeView, networks: ['all','ton','ethereum'] }
//   3. Pass onNativeBridgeOpen={key} from SwapTab if user selects TON
// No changes needed to SwapView logic below.
...
export function SwapView({ onBack, tokens: allTokens, balance, address, wallet, onRefresh, settings }: SwapViewProps) {
    const [nativeBridge, setNativeBridge] = useState<NativeBridgeKey | null>(null);

    const currentNetwork = settings?.network || 'all';
    const bridgeAllowed = NATIVE_BRIDGES.octra.networks.includes(currentNetwork) && !!wallet.evmAddress;
        ...
        return (
            <div className="swap-view-container animate-fade-in">
                <header className="view-header-minimal">
                    <button className="header-icon-btn" onClick={() => setNativeBridge(null)}>
    ...
    return (
        <div className="swap-view-container animate-fade-in">
            ...
            <SwapTab
                ...
                onBridgeTabOpen={bridgeAllowed ? () => setNativeBridge('octra') : undefined}
            />
        </div>
    );
}
