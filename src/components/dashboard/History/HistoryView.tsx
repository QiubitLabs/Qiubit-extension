...
import { getEthTransactions, getErc20Transactions, ethTxToTransaction } from '../../../services/network/EthHistoryService';
...
export function HistoryView({
    ...
    useEffect(() => {
        ...
        const fetchEthHistory = async () => {
            const [ethNative, ethTokens] = await Promise.all([
                getEthTransactions(evmAddress, 30),
                getErc20Transactions(evmAddress, 30)
            ]);
            if (cancelled) return;
            const converted = [
                ...ethNative.map(tx => ethTxToTransaction(tx, evmAddress, false)),
                ...ethTokens.map(tx => ethTxToTransaction(tx, evmAddress, true))
            ].sort((a, b) => {
    ...
    const allTransactions = useMemo(() => {
        const combined = [...transactions, ...ethTxs];
        // Deduplicate by hash
        ...
        return combined
            ...
            .sort((a, b) => {
                const aT = typeof a.timestamp === 'string' ? parseInt(a.timestamp) : a.timestamp;
                const bT = typeof b.timestamp === 'string' ? parseInt(b.timestamp) : b.timestamp;
                return bT - aT;
            });
    }, [transactions, ethTxs]);
    ...
    // Show detail page inline instead of modal
    if (selectedTx) {
        return (
    ...
    return (
        <div className="animate-fade-in">
            ...
            {/* Loading State - Only show full skeleton if NO data exists */}
