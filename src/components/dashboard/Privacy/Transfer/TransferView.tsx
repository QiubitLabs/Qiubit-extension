...
export function TransferView({
  ...
  const isList = activeView === "transfer_list";
  const displayTokens = tokenBalances.filter(
    (t: any) => t.encryptedBalance > 0,
  );
      ...
      <div className="token-list-view animate-slide-in">
        <header className="view-header-minimal mb-lg">
          <h3 className="text-sm font-semibold text-secondary uppercase tracking-wider">
            Select Token to Transfer Privately
          </h3>
        </header>
        {displayTokens.length === 0 ? (
          <div className="empty-state py-xl">
            <p className="text-secondary">
              No eligible tokens found for private transfer.
            </p>
          </div>
        ) : (
            ...
            {displayTokens.map((token: any) => (
              ...
              >
                ...
                <div className="token-balance-col">
                  <span className="token-bal-amount">
                    {formatAmount(token.encryptedBalance)}
                  </span>
                  <span className="text-xs text-secondary">
                    {token.symbol} (Encrypted)
                  </span>
                </div>
              </button>
            ))}
  ...
  if (activeView === "transfer_confirm") {
    const fee = 0.01; // Private transfers usually have fixed fee or specific estimation. Let's mock a standard fee for layout as requested.

    return (
      <div className="confirm-tx-page animate-fade-in relative">
        {/* Header like Send view */}
        <div className="flex items-center justify-between mb-xl">
          ...
          <div
            className="flex items-center justify-center gap-xs flex-1 bg-surface px-md py-1 rounded-full mx-auto"
            style={{ maxWidth: "fit-content" }}
          >
            <PrivateTransferIcon size={16} />
            <span className="font-bold text-sm tracking-wide">
              Private {selectedToken?.symbol}
            </span>
          </div>
          <button
            className="header-icon-btn"
        ...
        <div className="confirm-v2-title">
          Confirm{" "}
          <span className="confirm-v2-title-highlight">Private Transfer</span>
        </div>
        ...
        <div className="confirm-v2-amount-row">
          ...
          <span className="confirm-v2-amount-val">
        ...
        <div className="confirm-v2-details">
          <div className="confirm-v2-row align-top p-b-md">
            ...
            <div className="confirm-v2-value-col">
              ...
              <span className="confirm-v2-wallet-badge">
                {wallet?.name || "Wallet 01 - Akun 01"}
              </span>
            </div>
          </div>
        ...
        <div className="confirm-v2-btn-wrapper mt-auto pt-xl">
          ...
          >
            {isSubmitting ? "Sending Privately..." : "Confirm"}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="action-form-view animate-slide-in transfer">
      <header className="view-header-minimal mb-lg">
        <div className="flex items-center gap-sm">
          <PrivateTransferIcon size={18} className="text-accent" />
          <h3 className="text-sm font-semibold text-secondary uppercase tracking-wider">
            Private Transfer {selectedToken?.symbol}
          </h3>
        </div>
      </header>
      ...
      <form onSubmit={onSubmit} className="action-form">
        <div className="form-info-box mb-lg">
          <div className="info-row">
            <span className="info-label">Available (Encrypted):</span>
