...
type ActiveView = 'dashboard' | 'shield_list' | 'unshield_list' | 'transfer_list' | 'shield_form' | 'unshield_form' | 'transfer_form' | 'transfer_confirm' | 'claim_list';

export function PrivacyView({ wallet, onBack, showToast, publicBalance, onRefresh }: PrivacyViewProps) {
    const [encryptedBalance, setEncryptedBalance] = useState<EncryptedBalanceResult | null>(null);
    ...
    const [formData, setFormData] = useState<{ amount: string; recipient: string; contractData: string }>({ amount: '', recipient: '', contractData: '' });
    ...
    const fetchPrivacyData = useCallback(async (isInitial = false) => {
        ...
        try {
            ...
            const cachedResult = await privacyService.getEncryptedBalance(wallet.address, false);
            if (cachedResult.success) setEncryptedBalance(cachedResult);

            // 2. Trigger fresh fetch if we really need it (like on manual refresh) or if cache was used
            // But wait, fetchPrivacyData is called on mount with isInitial=true.
            // If isInitial=true, we want to show cache fast, then update.
            // If isRefreshing=true (manual), we want FORCE refresh.

            let finalResult = cachedResult;
            if (cachedResult.fromCache || isRefreshing) {
                const freshResult = await privacyService.getEncryptedBalance(wallet.address, true);
                if (freshResult.success) {
                    setEncryptedBalance(freshResult);
                    finalResult = freshResult;
                }
            } else {
                // If not from cache, it was a fresh fetch already (because cache missed)
            ...
            // Qiubit Native Token is always part of the list
            const allTokens = [
                {
                    symbol: 'OCT',
                    name: 'Octra',
    ...
    const handleShieldSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            setActiveView('dashboard');
            // Shield/Unshield affects both public and private balance
            onRefresh?.('both');
    ...
    const handleTransferSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            await privacyService.privacyTransfer(wallet.address, formData.recipient, amountNum);
            showToast?.('Private transfer sent', 'success');
            setActiveView('dashboard');
            // Private transfer only affects private balance
            onRefresh?.('private');
            setTimeout(() => fetchPrivacyData(), 2000);
        } catch (error: any) {
    ...
    const selectTokenForAction = (token: PrivacyToken, action: 'shield' | 'unshield' | 'transfer') => {
    ...
    const renderCurrentView = () => {
        switch (activeView) {
            case 'dashboard':
                return (
                    <PrivacyDashboard
                        totalEncryptedUsd={(encryptedBalance?.encryptedBalance || 0) * octPrice}
                        shieldedPercent={
                            (encryptedBalance?.totalBalance || 0) > 0
                                ? Math.round(((encryptedBalance?.encryptedBalance || 0) / (encryptedBalance?.totalBalance || 1)) * 100)
                                : 0
                        }
            ...
            case 'shield_form':
                return (
                    <ShieldView
                        ...
                        onTokenSelect={(token: PrivacyToken) => selectTokenForAction(token, 'shield')}
                        onAmountChange={(val: string) => setFormData({ ...formData, amount: val })}
                        onSetMax={(val: string) => setFormData({ ...formData, amount: val })}
            ...
            case 'unshield_form':
                return (
                    <UnshieldView
                        ...
                        formData={formData}
                        isSubmitting={isSubmitting}
                        onTokenSelect={(token: PrivacyToken) => selectTokenForAction(token, 'unshield')}
                        onAmountChange={(val: string) => setFormData({ ...formData, amount: val })}
                        onSetMax={(val: string) => setFormData({ ...formData, amount: val })}
                        onSubmit={handleShieldSubmit}
                    />
            ...
            case 'transfer_confirm':
                return (
                    <TransferView
                        ...
                        formData={formData}
                        isSubmitting={isSubmitting}
                        onTokenSelect={(token: PrivacyToken) => selectTokenForAction(token, 'transfer')}
                        onFormChange={(key: string, val: string) => setFormData({ ...formData, [key]: val })}
                        onSetMax={(val: string) => setFormData({ ...formData, amount: val })}
                        onReview={(e) => { e?.preventDefault(); setActiveView('transfer_confirm'); }}
    ...
    const getViewTitle = () => {
        switch (activeView) {
            case 'dashboard': return 'Privacy';
            case 'shield_list':
            case 'shield_form': return 'Shield Fund';
            case 'unshield_list':
            case 'unshield_form': return 'Unshield Fund';
            case 'transfer_list':
            case 'transfer_form': return 'Private Transfer';
            case 'claim_list': return 'Claim Funds';
            ...
            {activeView !== 'transfer_confirm' && (
                <div className="flex items-center gap-md mb-xl p-0">
                    ...
                    {activeView === 'dashboard' && (
                        <button className="header-icon-btn" onClick={() => fetchPrivacyData()} disabled={isRefreshing}>
