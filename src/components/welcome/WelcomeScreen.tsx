...
// import { AnimatedLogo } from './animations/AnimatedLogo'; // REMOVED
import { StepHeader } from './StepHeader/StepHeader';
import { calculatePasswordStrength } from '../../utils/validation';
import { Wallet } from '../../types';

// Import Lottie animations - REMOVED
...
function SuccessSplash({ onFinish }: SuccessSplashProps) {
    const [phase, setPhase] = useState<'entering' | 'visible' | 'exiting'>('entering'); // entering -> visible -> exiting
...
export function CreateWalletScreen({ onBack, onComplete }: CreateWalletScreenProps) {
    ...
    const handleSetPassword = async () => {
        ...
        try {
            ...
            const otherWords = currentMnemonic.filter((_, idx) => !positions.includes(idx + 1));
    ...
    const handleVerify = () => {
        ...
        try {
            ...
            rpc.getBalance(wallet.address).catch((e: any) => console.debug('Warmup ignored:', e));
        } catch (e) {
            ...
            {step === 1 && (
                <div className="create-password-step">
                    ...
                    <div className="step-content">
                        ...
                        {/* Description */}
                        <p className="step-description" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                            ...
                            <div className="privacy-policy-container">
                                <label className="privacy-policy-label">
                                    ...
                                    <span className="privacy-text">
                                        I agree to the <a href="https://octra.network/privacy" target="_blank" rel="noopener noreferrer" className="privacy-link">Privacy Policy & Terms</a>
                                    </span>
                                </label>
                            </div>
...
export function ImportWalletScreen({ onBack, onComplete }: ImportWalletScreenProps) {
    const [step, setStep] = useState(1);
    const [password, setPassword] = useState('');
    const [confirmPassword, setConfirmPassword] = useState('');
    const [showPassword, setShowPassword] = useState(false);
