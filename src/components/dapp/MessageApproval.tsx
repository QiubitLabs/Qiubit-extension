...
function renderTypedDataNode(data: any): React.ReactNode {
  if (data === null || data === undefined)
    return <span className="da-td-val-null">null</span>;
  ...
  if (typeof data === "boolean") {
    return (
      <span className={`da-td-val-bool ${data ? "true" : "false"}`}>
        {String(data)}
      </span>
    );
  }
  ...
  if (strVal.startsWith("0x") && strVal.length === 42) {
    return (
      <span className="da-td-val-addr mono" title={strVal}>
        {truncate(strVal, 8, 6)}
      </span>
    );
  }
...
export function MessageApproval({
  ...
  if (request.action === "ethPersonalSign") {
    ...
    displayMsg =
      request.params?.message || request.params?.payload?.message || "";
  } else if (isTypedData) {
  ...
  return (
    <div className="da-body-content da-message-layout">
        ...
        <div className="da-typed-data-container">
              ...
              {typedDataDomain.verifyingContract && (
                <div className="da-row">
                  ...
                  <span
                    ...
                    title={typedDataDomain.verifyingContract}
                  >
          ...
          {typedPrimaryType && (
            <div className="da-typed-primary-type-row">
              <span className="da-section-label">
                Contract Action (Primary Type):
              </span>
      ...
      <div className="da-notice warning da-message-warning">
        ...
        <span>
          {isTypedData
            ? "Signing this structured data can trigger on-chain actions (such as gasless transactions or token approvals). Only approve if you understand the consequences."
            : "Signing this message proves you own this wallet address. Never sign random or suspicious messages."}
        </span>
      </div>
    </div>
  );
