/**
 * LOGIC: Handles construction, canonical JSON serialization, hashing, and Ed25519 signature of Octra network transactions to match CLI/C++ node expectations.
 * EXPORTS:
 *   - TransactionPayload (interface)
 *   - canonicalJson (function)
 *   - signMessage (function)
 *   - verifySignature (function)
 *   - parseAmountToRaw (function)
 *   - createTransaction (async function)
 *   - computeTxHash (async function)
 *   - signBalanceRequest (function)
 *   - signRegisterPubkeyRequest (function)
 * FUNCTIONS:
 *   - jsonEscape(s): Escapes backslashes and quotes to produce compliant JSON strings.
 *   - formatTimestamp(ts): Formats timestamp as a double with ".0" for integers to match node expectations.
 *   - canonicalJson(tx): Serializes fields in a strict order (from, to_, amount, nonce, ou, timestamp, op_type, etc.) for signing.
 *   - signMessage(message, privateKeyB64): Signs a payload string/buffer with Ed25519 using TweetNaCl.
 *   - verifySignature(message, signature, publicKeyB64): Verifies an Ed25519 signature.
 *   - nowTimestamp(): Returns the current time in seconds.
 *   - parseAmountToRaw(amount): Multiplies amount by 10^6 (micro-units) with validation.
 *   - createTransaction(from, to, amount, nonce, privateKeyB64, message, fee, opType, encryptedData): Builds a payload, resolves/estimates fees (OU), and signs it.
 *   - computeTxHash(tx): Computes SHA-256 of canonical JSON transaction and returns hex.
 *   - signBalanceRequest(address, privateKeyB64): Signs authentication payload for fetching encrypted balance.
 *   - signRegisterPubkeyRequest(address, privateKeyB64): Signs authentication payload for registering public key.
 */
...
export function canonicalJson(tx: TransactionPayload): string {
  let s =
    ...
    ',"op_type":"' +
    jsonEscape(tx.op_type || "standard") +
    '"';
...
export function parseAmountToRaw(amount: string | number): string {
  ...
  const integerPart = s.substring(0, dot);
 ...
 */
export function signRegisterPubkeyRequest(
  address: string,
  privateKeyB64: string,
): string {
