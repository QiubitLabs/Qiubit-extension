 ...
 * Layer 2: Secure Storage (30s TTL, 5ms access)
...
class BalanceCache {
    ...
    async get(address: string, password?: string): Promise<CacheResult | null> {
        ...
        if (password) {
            try {
                ...
                if (storageCached) {
                    const age = storageCached.age || (Date.now() - (storageCached.timestamp || 0));

                    // STALE-WHILE-REVALIDATE:
                    // If data is visible (< 5 min), return it immediately!
                    if (age < this.DISPLAY_TTL) {
                        console.log(`[BalanceCache] Storage hit (${Math.round(age / 1000)}s old). Stale? ${age > this.FRESH_TTL}`);
     ...
     */
    async fetchWithDedup<T>(address: string, fetcher: (addr: string) => Promise<T>): Promise<T> {
