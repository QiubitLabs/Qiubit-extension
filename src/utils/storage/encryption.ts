/**
 * LOGIC: Provides storage vault encryption/decryption primitives using AES-GCM (Web Crypto API).
 * Implements a 5-pass secure memory wipe algorithm, PBKDF2 key derivation with 1,000,000 KDF iterations, constant-time HMAC-SHA256 integrity verifications, password hashing with static salts, and supports backward-compatibility for legacy v3 pipe-separated data formats.
 * EXPORTS:
 *   - VaultData (interface)
 *   - encryptDataSecure (async function)
 *   - decryptDataSecure (async function)
 * FUNCTIONS:
 *   - secureWipe(buffer): Overwrites a Uint8Array 5 times (zeros, secure random, zeros, 0xFF, zeros) in-memory to prevent cold-boot attacks.
 *   - generateHMAC(data, key): Computes SHA-256 HMAC of string payload returned as Base64.
 *   - verifyHMAC(data, mac, key): Compares HMACs in constant time to avoid timing side-channel attacks.
 *   - deriveKey(password, salt): Uses PBKDF2 with 1,000,000 iterations to derive a 256-bit AES-GCM key.
 *   - hashPassword(password): Generates a static-salted SHA-256 hex string for offline password comparison.
 *   - encryptDataSecure(data, password): Encrypts data with a dynamic 16-byte salt, 12-byte random IV, and adds an HMAC signature (v4 format).
 *   - decryptDataSecure(encryptedData, password): Verifies HMAC integrity and decrypts ciphertext. Handles v4 (JSON objects) and legacy v3 (pipe-separated string) formats.
 */

export interface VaultData {
  data: string;
  hmac: string;
  salt: string;
  version: number;
  algorithm: string;
  iterations: number;
}

/**
 * Secure memory wipe
 * Uses 5-pass wipe pattern for maximum security
 */
function secureWipe(buffer: Uint8Array | null | undefined): null {
  if (!buffer) return null;

  if (buffer instanceof Uint8Array) {
    buffer.fill(0);

    try {
      crypto.getRandomValues(buffer);
    } catch (e) {
      throw new Error(
        "Secure random number generation not available. Please use a modern browser.",
      );
    }

    buffer.fill(0);

    buffer.fill(0xff);

    buffer.fill(0);
  }

  return null;
}

/**
 * Generate HMAC-SHA256
 */
async function generateHMAC(data: string, key: string): Promise<string> {
  const encoder = new TextEncoder();
  const keyData = encoder.encode(key);

  const cryptoKey = await crypto.subtle.importKey(
    "raw",
    keyData,
    { name: "HMAC", hash: "SHA-256" },
    false,
    ["sign"],
  );

  const signature = await crypto.subtle.sign(
    "HMAC",
    cryptoKey,
    encoder.encode(data),
  );

  const bytes = new Uint8Array(signature);
  let binary = "";
  for (let i = 0; i < bytes.length; i++) {
    binary += String.fromCharCode(bytes[i]);
  }
  return btoa(binary);
}

/**
 * Verify HMAC (constant-time)
 */
async function verifyHMAC(
  data: string,
  mac: string,
  key: string,
): Promise<boolean> {
  const expectedMac = await generateHMAC(data, key);

  if (mac.length !== expectedMac.length) return false;

  let result = 0;
  for (let i = 0; i < mac.length; i++) {
    result |= mac.charCodeAt(i) ^ expectedMac.charCodeAt(i);
  }

  return result === 0;
}

/**
 * Derive encryption key from password using PBKDF2
 */
async function deriveKey(
  password: string,
  salt: Uint8Array | null = null,
): Promise<CryptoKey> {
  const encoder = new TextEncoder();
  const passwordBytes = encoder.encode(password);

  const keyMaterial = await crypto.subtle.importKey(
    "raw",
    passwordBytes,
    "PBKDF2",
    false,
    ["deriveBits", "deriveKey"],
  );

  const saltBytes = salt || encoder.encode("_x8f_kdf_salt_v3_");

  const key = await crypto.subtle.deriveKey(
    {
      name: "PBKDF2",
      salt: saltBytes as unknown as BufferSource,
      iterations: 1000000, // 1M iterations (OWASP recommendation)
      hash: "SHA-256",
    },
    keyMaterial,
    { name: "AES-GCM", length: 256 },
    false,
    ["encrypt", "decrypt"],
  );

  secureWipe(passwordBytes);
  return key;
}

/**
 * Encrypt data with HMAC and random salt (v4 format)
 * Security: Random salt per encryption prevents rainbow table attacks
 * Format: Returns object with { data, iv, salt, hmac, version }
 */
export async function encryptDataSecure(
  data: any,
  password: string,
): Promise<VaultData> {
  let key: CryptoKey | null = null;
  let iv: Uint8Array | null = null;
  let plaintext: Uint8Array | null = null;
  let salt: Uint8Array | null = null;

  try {
    salt = crypto.getRandomValues(new Uint8Array(16));

    key = await deriveKey(password, salt);

    iv = crypto.getRandomValues(new Uint8Array(12));

    const encoder = new TextEncoder();
    plaintext = encoder.encode(JSON.stringify(data));

    const ciphertext = await crypto.subtle.encrypt(
      { name: "AES-GCM", iv: iv as unknown as BufferSource },
      key,
      plaintext as unknown as BufferSource,
    );

    const combined = new Uint8Array(iv.length + ciphertext.byteLength);
    combined.set(iv);
    combined.set(new Uint8Array(ciphertext), iv.length);

    let binaryCombined = "";
    for (let i = 0; i < combined.length; i++) {
      binaryCombined += String.fromCharCode(combined[i]);
    }
    const combinedBase64 = btoa(binaryCombined);

    let binarySalt = "";
    for (let i = 0; i < salt.length; i++) {
      binarySalt += String.fromCharCode(salt[i]);
    }
    const saltBase64 = btoa(binarySalt);

    const hmacPayload = `${combinedBase64}:${saltBase64}:${4}`;
    const hmac = await generateHMAC(hmacPayload, password);

    return {
      data: combinedBase64,
      hmac: hmac,
      salt: saltBase64,
      version: 4,
      algorithm: "PBKDF2-SHA256",
      iterations: 1000000,
    };
  } catch (error) {
    console.error("[StorageSecure] Encryption failed:", error);
    throw new Error("Encryption failed");
  } finally {
    if (iv) secureWipe(iv);
    if (plaintext) secureWipe(plaintext);
    if (salt) secureWipe(salt);
  }
}

/**
 * Decrypt data with integrity check
 * Supports both v4 (object with random salt) and v3 (legacy pipe format)
 */
export async function decryptDataSecure(
  encryptedData: any,
  password: string,
): Promise<any> {
  let key: CryptoKey | null = null;
  let decrypted: ArrayBuffer | null = null;
  let salt: Uint8Array | null = null;

  try {
    if (typeof encryptedData === "object" && encryptedData.version === 4) {
      salt = Uint8Array.from(atob(encryptedData.salt), (c) => c.charCodeAt(0));

      const hmacPayload = `${encryptedData.data}:${encryptedData.salt}:${encryptedData.version}`;
      const isValid = await verifyHMAC(
        hmacPayload,
        encryptedData.hmac,
        password,
      );
      if (!isValid) {
        throw new Error(
          "Data integrity check failed: HMAC verification failed (v4)",
        );
      }

      key = await deriveKey(password, salt);

      const combined = Uint8Array.from(atob(encryptedData.data), (c) =>
        c.charCodeAt(0),
      );
      const iv = combined.slice(0, 12);
      const ciphertext = combined.slice(12);

      decrypted = await crypto.subtle.decrypt(
        { name: "AES-GCM", iv },
        key,
        ciphertext,
      );

      const decoder = new TextDecoder();
      const decodedResult = decoder.decode(decrypted);

      try {
        return JSON.parse(decodedResult);
      } catch (e) {
        return JSON.parse(decodeURIComponent(escape(decodedResult)));
      }
    }

    if (typeof encryptedData === "string") {
      const parts = encryptedData.split("|");
      if (parts.length !== 2) {
        throw new Error("Invalid encrypted data format");
      }

      const [combinedBase64, hmac] = parts;

      const isValid = await verifyHMAC(combinedBase64, hmac, password);
      if (!isValid) {
        throw new Error(
          "Data integrity check failed: HMAC verification failed (v3)",
        );
      }

      key = await deriveKey(password, null);
      const combined = Uint8Array.from(atob(combinedBase64), (c) =>
        c.charCodeAt(0),
      );

      const iv = combined.slice(0, 12);
      const ciphertext = combined.slice(12);

      decrypted = await crypto.subtle.decrypt(
        { name: "AES-GCM", iv },
        key,
        ciphertext,
      );

      const decoder = new TextDecoder();
      const result = JSON.parse(decoder.decode(decrypted));

      return result;
    }

    throw new Error("Unsupported vault format");
  } catch (error: any) {
    const errorName = error?.name || "Error";
    const errorMsg = error?.message || String(error);
    const errorStack = error?.stack || "";

    console.error(
      `[StorageSecure] Decryption failed (${errorName}): ${errorMsg}`,
    );
    if (errorStack) console.error(errorStack);

    if (errorMsg.includes("integrity") || errorName.includes("Integrity")) {
      throw error;
    }
    throw new Error(`Decryption failed: ${errorName} - ${errorMsg}`);
  } finally {
    secureWipe(salt);
  }
}
