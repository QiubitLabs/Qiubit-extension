...
const TEST_PAYLOAD = { wallets: [{ address: 'oct1abc', privateKeyB64: 'c2VjcmV0S2V5QmFzZTY0' }] };
...
describe('HMAC integrity — hard fail', () => {
    it('throws when HMAC is tampered (v4)', async () => {
        ...
        // Corrupt the HMAC by flipping the first character
        const tampered = {
            ...encrypted,
            hmac: ('X' + encrypted.hmac.slice(1))
        };

        await expect(decryptDataSecure(tampered as any, PASSWORD))
            .rejects
            .toThrow(/integrity|HMAC/i);
    });

    it('throws when HMAC is entirely replaced (v4)', async () => {
        const encrypted = await encryptDataSecure(TEST_PAYLOAD, PASSWORD);
        const tampered = { ...encrypted, hmac: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa=' };
    ...
    it('throws when ciphertext is tampered (v4)', async () => {
        ...
        const tamperedBytes = dataBytes.slice(0, idx) +
        ...
        await expect(decryptDataSecure(tampered as any, PASSWORD))
            .rejects
            .toThrow(/integrity|HMAC|Decryption/i);
    });

    it('throws when salt is replaced (v4)', async () => {
        const encrypted = await encryptDataSecure(TEST_PAYLOAD, PASSWORD);
        const fakeSalt = btoa(String.fromCharCode(...new Uint8Array(16).fill(0xAB)));
    ...
    it('does NOT silently succeed on wrong password — must throw', async () => {
        ...
        await expect(decryptDataSecure(encrypted as any, WRONG_PASSWORD))
            .rejects
            .toThrow();
    });
});
    ...
    it('encrypts and decrypts large payloads', async () => {
        const largePayload = {
            wallets: Array.from({ length: 50 }, (_, i) => ({
                address: `oct${i}`, privateKeyB64: btoa('x'.repeat(32))
            }))
        };
...
describe('Key derivation properties', () => {
    ...
    it('v4 vault stores algorithm and iterations metadata', async () => {
        ...
        // Algorithm field documents the KDF used (PBKDF2-SHA256 or AES-GCM depending on impl)
...
describe('HMAC bypass regression', () => {
    it('does NOT return data when HMAC fails — regression for soft-warning bypass', async () => {
        ...
        // Must not have returned the secret
