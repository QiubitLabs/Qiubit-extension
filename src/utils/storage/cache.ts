/**
 * LOGIC: Coordinates encrypted and unencrypted key-value caching in persistent storage.
 * Enforces AES-GCM password-based encryption for sensitive wallet assets (balances, tokens, custom tokens, private balances) and permits plaintext JSON for public config caches.
 * EXPORTS:
 *   - saveBalanceCacheSecure / getBalanceCacheSecure (async functions)
 *   - saveTokenCacheSecure / getTokenCacheSecure (async functions)
 *   - saveCustomTokensSecure / loadCustomTokensSecure (async functions)
 *   - savePrivacyBalanceCacheSecure / getPrivacyBalanceCacheSecure / clearPrivacyBalanceCacheSecure (async functions)
 *   - savePublicCache / getPublicCache (async functions)
 * FUNCTIONS:
 *   - saveBalanceCacheSecure(address, data, password) / getBalanceCacheSecure(address, password): Encrypts/decrypts the balance associated with the account.
 *   - saveTokenCacheSecure(address, tokens, password) / getTokenCacheSecure(address, password): Encrypts/decrypts discovered token lists.
 *   - saveCustomTokensSecure(tokens, password) / loadCustomTokensSecure(password): Encrypts/decrypts custom user tokens.
 *   - savePrivacyBalanceCacheSecure(address, data, password) / getPrivacyBalanceCacheSecure(address, password): Encrypts/decrypts FHE-based encrypted private balances.
 *   - clearPrivacyBalanceCacheSecure(address): Removes the private balance cache entry.
 *   - savePublicCache(key, data) / getPublicCache(key): Encodes/decodes unencrypted JSON.
 */
 ...
 */
export async function saveBalanceCacheSecure(
  address: string,
  data: any,
  password: string,
): Promise<void> {
 ...
 */
export async function getBalanceCacheSecure(
  address: string,
  password: string,
): Promise<any | null> {
 ...
 */
export async function saveTokenCacheSecure(
  address: string,
  tokens: any[],
  password: string,
): Promise<void> {
 ...
 */
export async function getTokenCacheSecure(
  address: string,
  password: string,
): Promise<any[] | null> {
  if (!password || !address) return null;
 ...
 */
export async function saveCustomTokensSecure(
  tokens: any,
  password: string,
): Promise<void> {
  if (!password) return;
 ...
 */
export async function savePrivacyBalanceCacheSecure(
  address: string,
  data: any,
  password: string,
): Promise<void> {
 ...
 */
export async function getPrivacyBalanceCacheSecure(
  address: string,
  password: string,
): Promise<any | null> {
  if (!password || !address) return null;
 ...
 */
export async function clearPrivacyBalanceCacheSecure(
  address: string,
): Promise<void> {
  if (!address) return;
