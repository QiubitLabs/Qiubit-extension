 ...
 */
export function getChainConfig(networkIdOrChainId: string | number | null): BaseChainConfig | null {
...
export function getInsufficientFundsMessage(networkIdOrChainId: string | number | null): string {
    const chain = getChainConfig(networkIdOrChainId);
    return chain ? chain.getInsufficientFundsMessage() : 'Insufficient balance for gas fee.';
}
