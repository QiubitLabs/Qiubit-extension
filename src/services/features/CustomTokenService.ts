...
export async function getCustomTokens(walletAddress: string, chainId?: number): Promise<CustomToken[]> {
    const all = await readAll();
    const list = all[walletAddress] ?? [];
    return chainId != null ? list.filter(t => t.chainId === chainId) : list;
}

export async function addCustomToken(walletAddress: string, token: CustomToken): Promise<void> {
    const all = await readAll();
    const list = all[walletAddress] ?? [];
    const exists = list.findIndex(t =>
...
export async function removeCustomToken(
    ...
    const all = await readAll();
    const list = all[walletAddress] ?? [];
    all[walletAddress] = list.filter(t =>
        !(t.contractAddress.toLowerCase() === contractAddress.toLowerCase() && t.chainId === chainId)
    );
    await writeAll(all);
}
 ...
 */
async function fetchLogoFromLiFi(contractAddress: string, chainId: number): Promise<string> {
    try {
...
export async function fetchTokenMetadata(
    ...
    // Confirm the contract exists on this chain first — throws if not found.
    // Only then fetch the logo, so we don't waste LI.FI requests on wrong chains
    // when AddTokenModal probes multiple networks in parallel.
...
function resolveTokenLogoTrustWallet(contractAddress: string, chainId: number): string {
