...
class DappServiceClass {
    ...
    async saveConnections() {
        try {
            ...
            await storage.set({ [STORAGE_KEY]: JSON.stringify(connections) });
        } catch (error) {
     ...
     */
    async addConnection(origin: string, info: any) {
     ...
     * Remove connection
     */
    async removeConnection(origin: string) {
     ...
     */
    addPendingRequest(requestId: string, request: any) {
