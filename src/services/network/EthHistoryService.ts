...
// networkId → Moralis chain identifier
...
// ─── eth_getLogs fallback constants ───────────────────────────────────────────
...
// In-memory API result cache — prevents hitting Moralis again within 5 min.
// Key: "native|erc20:<networkId>:<address>" → { ts, data }
...
function _apiSet(key: string, data: EthTransaction[]): void {
    if (_apiCache.size > 100) {
        const now = Date.now();
        for (const [k, v] of _apiCache) if (now - v.ts > API_CACHE_TTL) _apiCache.delete(k);
    }
    _apiCache.set(key, { ts: Date.now(), data });
}

// Last-known Moralis timestamp per address+chain — used for incremental fetches.
const LS_MLAST = 'octra_mlast:';
function getMoralisLastTs(key: string): string | undefined {
    try { return localStorage.getItem(LS_MLAST + key) ?? undefined; } catch { return undefined; }
}
function setMoralisLastTs(key: string, isoTs: string): void {
    try { localStorage.setItem(LS_MLAST + key, isoTs); } catch { /* ignore */ }
}

// ─── Chain activity tracker ───────────────────────────────────────────────────
// Per address+chain+type, records when the endpoint last returned 0 results.
// Any chain/endpoint confirmed empty is skipped for 24 h to avoid wasting CUs.
// Native (25 CU) and ERC20 (200 CU) are tracked independently.
...
export function markEndpointEmpty(tag: string): void {
    try { localStorage.setItem(LS_EMPTY + tag, String(Date.now())); } catch {}
}
/** Remove empty mark — endpoint has confirmed activity again. */
export function clearEndpointEmpty(tag: string): void {
    try { localStorage.removeItem(LS_EMPTY + tag); } catch {}
}
...
function setLastBlock(key: string, block: number): void {
    try { localStorage.setItem(LS_LB + key, String(block)); } catch { /* storage full */ }
}

// One-time reset so stale lastBlock entries don't interfere with the new Moralis flow.
const RESET_MIGRATION_KEY = 'octra_lb_reset_v5';
...
async function moralisFetch(path: string, params: Record<string, string>): Promise<any> {
...
async function fetchMoralisNative(
    ...
    try {
        const params: Record<string, string> = { chain, limit: String(limit), order: 'DESC' };
...
async function ankrPost(method: string, params: Record<string, unknown>): Promise<any> {
...
// ─── eth_getLogs fallback (Monad, Hyperliquid, etc.) ─────────────────────────

async function getWorkingProvider(chainId: number): Promise<ethers.JsonRpcProvider | null> {
...
async function hydrateTokenMeta(provider: ethers.JsonRpcProvider, contracts: string[]): Promise<void> {
    const missing = [...new Set(contracts.map(a => a.toLowerCase()))].filter(a => !tokenMeta.has(a));
    for (let i = 0; i < missing.length; i += 5) {
        await Promise.all(missing.slice(i, i + 5).map(async addr => {
            try {
...
async function hydrateBlockTs(provider: ethers.JsonRpcProvider, blockNumbers: number[]): Promise<void> {
    const missing = [...new Set(blockNumbers)].filter(n => !blockTs.has(n));
    for (let i = 0; i < missing.length; i += 5) {
        await Promise.all(missing.slice(i, i + 5).map(async bn => {
            try {
...
async function fetchLogsErc20(
    ...
    const scanFrom = savedBlock !== undefined
        ? savedBlock + 1
    ...
    let chunks = 0;
    for (let hi = latest; hi >= scanFrom && chunks < effectiveMaxChunks; hi -= CHUNK, chunks++) {
        const lo = Math.max(scanFrom, hi - CHUNK + 1);
        const [sent, recv] = await Promise.all([
            provider.getLogs({ topics: [TRANSFER_SIG, paddedAddr], fromBlock: lo, toBlock: hi }).catch(() => [] as ethers.Log[]),
            provider.getLogs({ topics: [TRANSFER_SIG, null, paddedAddr], fromBlock: lo, toBlock: hi }).catch(() => [] as ethers.Log[]),
        ]);
    ...
    unique.sort((a, b) => b.blockNumber - a.blockNumber || (a as any).logIndex - (b as any).logIndex);
    const finalLogs = unique.slice(0, limit);

    await Promise.all([
        hydrateBlockTs(provider, finalLogs.map(l => l.blockNumber)),
        hydrateTokenMeta(provider, finalLogs.map(l => l.address)),
    ]);
    ...
    return finalLogs.map(log => {
        ...
        const meta = tokenMeta.get(log.address.toLowerCase()) ?? { symbol: '???', decimals: 18 };
...
export async function getEthTransactions(
    ...
    // 1. Moralis (with incremental from_date after first fetch)
    ...
    if (moralisChain && MORALIS_KEY) {
        ...
        const fresh = await fetchMoralisNative(evmAddress, limit, moralisChain, fromDate);
        if (fresh.length > 0) {
            const latestTs = Math.max(...fresh.map(t => parseInt(t.timeStamp) * 1000));
    ...
    if (ankrChain && ANKR_KEY) {
        ...
        if (r.length > 0) { _apiSet(cKey, r); return r; }
    }

    // 3. Etherscan-compatible (Sepolia)
    if (net.historyApi?.type === 'etherscan_compatible' && net.historyApi.baseUrl) {
        const params = new URLSearchParams({
            module: 'account', action: 'txlist', address: evmAddress,
            startblock: '0', endblock: 'latest', sort: 'desc',
            page: '1', offset: String(limit),
        });
        try {
            ...
            const r = data.result.map((tx: any) => ({
                ...
                value: tx.value, timeStamp: tx.timeStamp, isError: tx.isError,
            }));
            _apiSet(cKey, r);
            return r;
        } catch { return []; }
...
export async function getErc20Transactions(
    ...
    if (moralisChain && MORALIS_KEY) {
        ...
        const fromDate = getMoralisLastTs(lKey);
        const fresh = await fetchMoralisErc20(evmAddress, limit, moralisChain, fromDate);
        if (fresh.length > 0) {
            const latestTs = Math.max(...fresh.map(t => parseInt(t.timeStamp) * 1000));
            setMoralisLastTs(lKey, new Date(latestTs).toISOString());
        }
    ...
    if (net.historyApi?.type === 'etherscan_compatible' && net.historyApi.baseUrl) {
        const params = new URLSearchParams({
            module: 'account', action: 'tokentx', address: evmAddress,
            startblock: '0', endblock: 'latest', sort: 'desc',
            page: '1', offset: String(limit),
        });
        try {
            ...
            const r = data.result.map((tx: any) => ({
                ...
                tokenSymbol: tx.tokenSymbol, tokenDecimal: tx.tokenDecimal,
                contractAddress: tx.contractAddress, timeStamp: tx.timeStamp,
            }));
            _apiSet(cKey, r);
            return r;
        } catch { return []; }
    ...
    // 4. eth_getLogs for chains with no API support (Monad, Hyperliquid, etc.)
...
export async function getSepoliaTransactions(evmAddress: string, limit = 30): Promise<EthTransaction[]> {
