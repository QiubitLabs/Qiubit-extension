...
class RPCClient {
  ...
  constructor(rpcUrl: string = DEFAULT_RPC) {
    ...
    logInfo(
      `[RPCClient] Init: isExtension=${isExtension}, isDev=${isDev}, passedUrl=${rpcUrl}`,
    );

    if (!isExtension && isDev) {
      logInfo("[RPCClient] Dev mode (browser) - using /api proxy");
      this.rpcUrl = "/api";
    } else {
      this.rpcUrl = rpcUrl || DEFAULT_RPC;
    }

    logInfo(`[RPCClient] Actual RPC URL: ${this.rpcUrl}`);
  ...
  async request<T = any>(
    ...
    if (blockedUntil > Date.now()) {
      ...
      console.warn(
        `[RPC ${host}] Rate-limited — waiting ${Math.ceil(wait / 1000)}s before next request`,
      );
      return {
        status: 429,
        text: "rate-limited",
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        console.warn(
          `[RPC ${host}] 429 Too Many Requests — circuit breaker activated for 90s`,
        );
        return {
          status: 429,
          text,
      ...
      if (
        ...
        console.warn(
          `[RPC] Server error ${response.status}, retrying in ${delay}ms... (${retries} retries left)`,
        );
      ...
      if (error.name === "AbortError") {
        return {
          ...
          json: null,
          ok: false,
          error: "Request timeout (Check if RPC is UP or CORS blocked)",
        };
      }
        ...
        for (const fallbackUrl of FALLBACK_OCTRA_RPCS) {
          ...
          try {
            ...
            if (fallbackResp.ok) {
              ...
              try {
                fallbackJson = fallbackText.trim()
      ...
      if (retries > 0) {
        ...
        console.warn(
          `[RPC] Network error, retrying in ${delay}ms... (${retries} retries left)`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
        return this.request<T>(
          method,
   ...
   */
  async jsonRpcBatchCall(
    calls: { method: string; params: any[] }[],
  ): Promise<RPCResponse<any[]>> {
    if (calls.length === 0)
      return { status: 200, text: "[]", json: [], ok: true };
  ...
  async getAddressInfo(
    ...
    const result = await this.jsonRpcCall<any>("octra_transactionsByAddress", [
      address,
      limit,
    ...
    if (result.ok && result.json) {
      const txs = Array.isArray(result.json)
        ? result.json
  ...
  async sendBatchTransaction(txs: any[]): Promise<TransactionResult> {
    ...
    const error =
      result.json?.error || result.text || "Batch transaction failed";
    throw new Error(error);
  }
  ...
  async getBalanceRaw(
    address: string,
  ): Promise<{ nonce: number; balanceRaw: string; pendingNonce: number }> {
    try {
      ...
      if (result.ok && result.json) {
        ...
        const pendingNonce = parseSafeNumber(
          data.pending_nonce ?? data.nonce ?? 0,
        );
        let balanceRaw = "0";
  ...
  async getAccount(address: string, limit: number = 20): Promise<any> {
    const result = await this.jsonRpcCall<any>("octra_account", [
      address,
      limit,
    ]);
  ...
  async getEncryptedBalance(
    ...
    try {
      const result = await this.jsonRpcCall<any>("octra_encryptedBalance", [
        address,
        sigB64,
        pubB64,
      ]);
  ...
  async getEncryptedCipher(address: string): Promise<any> {
    try {
      const result = await this.jsonRpcCall<any>("octra_encryptedCipher", [
        address,
      ]);
      return result.ok ? result.json : null;
    } catch {
  ...
  async registerPvacPubkey(
    ...
    const result = await this.jsonRpcCall<any>("octra_registerPvacPubkey", [
      address,
      pkB64,
      sigB64,
      pubB64,
  ...
  async registerPublicKey(
    ...
    const result = await this.jsonRpcCall<any>("octra_registerPublicKey", [
      address,
      pubB64,
      sigB64,
    ]);
  ...
  async getStealthOutputs(fromEpoch: number = 0): Promise<any[]> {
    try {
      const result = await this.jsonRpcCall<any>("octra_stealthOutputs", [
        fromEpoch,
      ]);
      if (result.ok && result.json) {
        return Array.isArray(result.json) ? result.json : [];
  ...
  async callContractView(
    ...
    try {
      const result = await this.jsonRpcCall<any>(
        "contract_call",
        [contractAddress, method, params, callerAddress],
        15000,
      );
      return result.ok ? result.json : null;
    } catch (err) {
  ...
  /**
   * Auto-discover all OCS-01 tokens held by an address.
   * JSON-RPC: octra_tokensByAddress → { tokens: [{ address, name, symbol,
   * decimals, balance, total_supply, owner }], count, wallet_address }.
   * balance and decimals come back as raw strings.
   */
  ...
  > {
    try {
      ...
      const tokens = Array.isArray(result.json)
        ? result.json
        : result.json.tokens || [];
      return tokens
        .filter((t: any) => t && t.address)
        .map((t: any) => ({
          address: t.address,
          name: t.name || t.symbol || t.address.slice(0, 12),
          symbol: t.symbol || "OCS01",
          decimals: parseInt(String(t.decimals ?? "6"), 10) || 6,
          balanceRaw: String(t.balance ?? "0"),
          owner: t.owner,
        }));
    } catch (err) {
      console.warn("[RpcService] getTokensByAddress failed:", err);
      return [];
    }
  }

  /**
   * OCS-01 token transfer history for an address.
   * JSON-RPC: octra_tokenTransfersByAddress → { transactions, count, offset,
   * limit, total, has_more, incoming, outgoing }.
   */
  async getTokenTransfersByAddress(
    ...
    try {
      ...
      console.warn("[RpcService] getTokenTransfersByAddress failed:", err);
      return [];
    }
  }

  async getBridgeStatus() {
    return this.request("POST", "/api/bridge/signer", {
      method: "bridgeStatus",
    });
  }
