...
import { NETWORK_REGISTRY, type NetworkConfig } from '../../constants/networks/registry';
...
export function getAllNetworks(): Record<string, NetworkConfig> {
    ...
    const userEntries = userNets.reduce<Record<string, NetworkConfig>>((acc, n) => {
...
export function resolveNetworkByChainId(chainId: number): NetworkConfig | null {
    // Search registry first (O(n) but n is small)
    const builtin = Object.values(NETWORK_REGISTRY).find(n => n.chainId === chainId);
...
export function resolveNetworkForToken(token: {
    ...
    if (token.isNative || (!token.isEVM && !token.chainId)) return NETWORK_REGISTRY.octra;
    if (token.chainId) return resolveNetworkByChainId(token.chainId) ?? NETWORK_REGISTRY.ethereum;
