...
function read4byteCache(): FourByteCache {
  try {
    return JSON.parse(localStorage.getItem(FOURBYTE_CACHE_KEY) || "{}");
  } catch {
    return {};
  }
}
...
function write4byteCache(cache: FourByteCache): void {
  try {
    localStorage.setItem(FOURBYTE_CACHE_KEY, JSON.stringify(cache));
  } catch {
    /* quota — ignore */
  }
}
...
export async function decodeTx(
  ...
  try {
    ...
    if (abi && abi.length > 0) {
      ...
      const parsed: TransactionDescription | null = iface.parseTransaction({
        data: calldata,
      });
      if (parsed) {
