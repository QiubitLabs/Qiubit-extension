...
const MORALIS_CHAIN: Record<number, string> = {
    1: '0x1', 56: '0x38', 137: '0x89', 8453: '0x2105', 42161: '0xa4b1',
};
const ANKR_CHAIN: Record<number, string> = {
    1: 'eth', 56: 'bsc', 137: 'polygon', 8453: 'base', 42161: 'arbitrum',
};

// TrustWallet chain name for logo fallback
const TRUSTWALLET_CHAIN: Record<number, string> = {
    1: 'ethereum', 56: 'smartchain', 137: 'polygon',
    8453: 'base', 42161: 'arbitrum', 11155111: 'ethereum',
};
...
async function readCache(key: string): Promise<DiscoveredToken[] | null> {
    try {
        if (typeof chrome !== 'undefined' && chrome.storage?.local) {
            ...
            const c = res[key] as { ts: number; tokens: DiscoveredToken[] } | undefined;
...
async function writeCache(key: string, tokens: DiscoveredToken[]): Promise<void> {
    try {
        if (typeof chrome !== 'undefined' && chrome.storage?.local) {
...
function trustWalletLogo(chainId: number, checksumAddr: string): string | undefined {
...
async function fetchMoralisTokens(address: string, chainId: number): Promise<DiscoveredToken[] | null> {
    ...
    try {
        ...
        return items
            ...
            .map(t => {
                ...
                let bal = '0';
                try { bal = Number(ethers.formatUnits(BigInt(raw), dec)).toFixed(dec > 6 ? 4 : 2); } catch { /* ignore */ }
                let addr = t.token_address ?? '';
                try { addr = ethers.getAddress(addr); } catch { /* ignore */ }
                return {
                    chainId,
...
async function fetchAnkrTokens(address: string, chainId: number): Promise<DiscoveredToken[] | null> {
    ...
    try {
        ...
        return assets
            .filter(a => a.tokenType === 'ERC20' && a.balanceRawInteger && a.balanceRawInteger !== '0')
            .map(a => {
...
// ─── eth_getLogs discovery (Monad, Hyperliquid, etc.) ─────────────────────────

async function fetchLogsTokens(address: string, chainId: number): Promise<DiscoveredToken[]> {
    ...
    for (let hi = latest; hi >= scanFrom; hi -= LOGS_CHUNK) {
        ...
        [...sent, ...recv].forEach(l => foundContracts.add(l.address.toLowerCase()));
    ...
    const results = await Promise.all([...foundContracts].map(async addr => {
        try {
            ...
            const bal = Number(ethers.formatUnits(rawBal, dec)).toFixed(dec > 6 ? 4 : 2);
            return {
                chainId,
...
export async function discoverTokens(
    ...
    // 3. eth_getLogs fallback (Monad, Hyperliquid, any non-API chain)
    if (!tokens?.length) {
