...
export function useWalletState(password: string | null) {
  ...
  const handleAddWallet = useCallback(
    async (newWallet: Wallet, pwd: string) => {
      ...
      keyringService.addKey(
        newWallet.address,
        newWallet.privateKeyB64,
        newWallet.publicKeyB64,
      );
  ...
  const loadWalletsData = useCallback(async (pwd: string) => {
    ...
    let needSave = false;
    const patchedWallets = await Promise.all(
      loadedWallets.map(async (w) => {
        let patchedWallet = { ...w };
          ...
          try {
            ...
            const hdIndex =
              typeof patchedWallet.hdIndex === "number"
                ? patchedWallet.hdIndex
                : 0;
            const deriveSolanaKeysFromMnemonic = (
              await import("../utils/crypto/keys")
          ...
          try {
            ...
            const hdIndex =
              typeof patchedWallet.hdIndex === "number"
                ? patchedWallet.hdIndex
                : 0;
            const deriveSuiKeysFromMnemonic = (
              await import("../utils/crypto/keys")
            ).deriveSuiKeysFromMnemonic;
            const suiKeys = await deriveSuiKeysFromMnemonic(
              mnemonicStr,
              hdIndex,
            );
            needSave = true;
          ...
          try {
            ...
            const hdIndex =
              typeof patchedWallet.hdIndex === "number"
                ? patchedWallet.hdIndex
                : 0;
            const deriveBitcoinKeysFromMnemonic = (
              await import("../utils/crypto/keys")
        ...
        if (
          ...
          try {
            ...
            if (/^[a-fA-F0-9]{64}$/.test(pk)) {
              ...
              const base58Encode = (await import("../utils/crypto/format"))
                .base58Encode;
              const bufferToHex = (await import("../utils/crypto/format"))
                .bufferToHex;
              ...
              if (!patchedWallet.bitcoinAddress) {
                ...
                const toBech32Address = (
                  await import("../utils/crypto/derivation/bitcoin")
                ).toBech32Address;
            ...
            console.error(
              "Failed to patch Solana/Sui/Bitcoin address for private key wallet",
              e,
            );
          }
        }
    ...
    const activeIdx =
      savedIndex >= 0 && savedIndex < patchedWallets.length ? savedIndex : 0;
