...
export function useWalletRecovery({
    ...
    const handleRecover = useCallback(async ({ type, value, newPassword }: { type: 'mnemonic' | 'privateKey'; value: string; newPassword: string; }) => {
        try {
            ...
            // Using addWallet from storageSecure as we are in recovery mode (fresh start)
    ...
    }, [rpcClient, saveActiveSession, setView, setWallets, setPassword, setIsUnlocked, setBalance, setNonce]);
