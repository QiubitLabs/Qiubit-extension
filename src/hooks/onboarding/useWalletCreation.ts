...
export function useWalletCreation({
  ...
  const handleSetupPassword = useCallback(
    async (newPassword: string) => {
      ...
      if (pendingWallet) {
        ...
        setWallets([
          { ...pendingWallet, id: crypto.randomUUID(), name: "Wallet 1" },
        ]);
  ...
  const handleWalletGenerated = useCallback(
    async (newWallet: Wallet, newPassword: string) => {
      try {
        if (!(await hasPassword())) {
          try {
            ...
            console.warn(
              "[useWalletCreation] Could not force clear:",
              clearErr,
            );
          }
        }
        ...
        try {
          ...
          if (
            ...
            console.warn(
              "[useWalletCreation] Wallet add skipped (duplicate/race-condition), proceeding to login...",
            );
          } else {
            throw addErr;
          }
