...
export function useWalletImport({
    ...
    const handleImportWallet = useCallback(async (importedWallet: Wallet, newPassword: string) => {
        try {
            ...
            // 1. Initialize Keyring FIRST if needed (Critical for first-time import)
            if (!keyringService.isUnlocked()) {
                await keyringService.initialize([], passToUse);
            }

            // 2. Add Wallet (Now safe to call keyringService.addKey)
            try {
                await addWalletInternal(importedWallet, passToUse);
            } catch (addErr: any) {
                if (addErr.message && addErr.message.includes('Wallet already exists')) {
                    console.warn('[useWalletImport] Import ignored (duplicate), proceeding...');
                } else {
                    throw addErr;
                }
            }
    ...
    const handleAddWalletFromModal = useCallback(async (data: { type: string, privateKey?: string, mnemonic?: string }) => {
        ...
        // Delegate to handleImportWallet which handles adding to existing state via addWalletInternal
